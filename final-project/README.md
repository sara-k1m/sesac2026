...
| 날짜 | 목표 | 세부 작업 |
|---|---|---|
| **8/10(월)** | 전처리 · 상관분석 마무리 | ~~데이터 통합/전처리~~, ~~지오코딩~~, ~~금리·지하철 결합~~, 대표 금리 선정 및 Lag 분석(진행 중) |
| **8/11(화)** | 상관관계 분석 | 거리구간별(0-250m/250-500m/500m-1km/1km+) 가격 비교, 자치구별 금리 민감도 비교 |
| **8/12(수)** | 시기별 분석 | 금리 상승기/하락기 구간 나눠 가격 추이 비교, Lag 반영 전후 비교 |
| **8/13(목)** | 시각화 · 인사이트 정리 | 핵심 그래프 정리, 발표자료(PPT) 초안 작성 |
| **8/14(금)** | 최종 마무리 | 결과 리뷰, README/코드 정리, GitHub 업로드, 발표 |
